...
. Check that the value from the ConfigMap exists as an ENV variable in the pods container. First, exec into the container:

  $ kubectl exec -it app-pod  bash

. Once inside the container, note the `COUNT` variable:
