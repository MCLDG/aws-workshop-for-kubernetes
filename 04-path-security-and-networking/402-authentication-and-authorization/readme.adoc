...
== IAM container roles using kube2iam for EKS

The process of installing kube2iam on an EKS cluster differs in a couple of ways:

* RBAC is enabled, so RBAC service roles must be installed
* EKS uses VPC networking and this must be specified in the kube2iam configuration (see the file kube2iam-ds-eks.yaml)

=== Install kube2iam on EKS

The following steps describe how to configure kube2iam on EKS. Before installing kube2iam we need to allow our EKS worker nodes
to assume other IAM roles. kube2iam will use this to allow EKS pods to assume roles we assign.

. In the AWS IAM console, find the role for your EKS worker node. It will be named something like this `EKS-<cluster name>-DefaultNodeGroup-NodeInstanceRole-34NM9A2E90HE`
. In the Permissions tab, add an inline policy as follows, with a name of `eks-sts`:
...
. Back in your terminal, deploy the RBAC service accounts:
+
    $ kubectl apply -f templates/kube2iam-rbac.yaml
...
. Deploy kube2iam:
    ...
    $ kubectl apply -f templates/kube2iam-ds-eks.yaml
    daemonset.apps "kube2iam" created

=== Get the IAM role for the EKS worker nodes

We need the link:https://docs.aws.amazon.com/general/latest/gr/aws-arns-and-namespaces.html[Amazon Resource Name (ARN)] of the IAM Role assigned to the worker nodes.  We will need this as part of creating pod roles.

This is the same role you updated in the IAM console in the previous step. Note that we need the value of the Role ARN field.

Once you arrive at this point, continue at the step link:#create-the-pod-iam-role[Create the Pod IAM role] below.

== IAM container roles using kube2iam for Kops
...
. Deploy kube2iam as a DaemonSet. You will use a different YAML file if you are using EKS:
+
    $ kubectl apply -f templates/kube2iam-ds.yaml
    daemonset "kube2iam" created
+
    For EKS and for clusters where RBAC is enabled:
+
    $ kubectl apply -f templates/kube2iam-rbac.yaml
    $ kubectl apply -f templates/kube2iam-ds-eks.yaml
    daemonset "kube2iam" created
+
Note that the `kube2iam-ds.yaml` is assuming that your network configuration is based on `kubenet`, which is also the default kops configuration. If you have changed this as part of another exercise (e.g. switched to Calico) you will need to change the value of the `--host-interface` arg in the configuration file. For example, set to `cal+` if using Calico, or `eni+` if using AWS VPC networking.

=== IAM roles for worker nodes for Kops
...
== Create the Pod IAM role

Edit the `templates/pod-role-trust-policy.json` file, replace `{{NodeIAMRoleARN}}` with the IAM Role ARN obtained from the previous step, i.e. the ARN of the role used by the Kubernetes worker nodes.
...
=== Cleanup
    ...
    $ kubectl delete -f templates/pod-with-kube2iam.yaml
    $ aws iam detach-role-policy --role-name MyPodRole --policy-arn arn:aws:iam::aws:policy/AmazonS3ReadOnlyAccess
    $ aws iam delete-role --role-name MyPodRole
    $ kubectl delete -f templates/kube2iam-ds-eks.yaml
    $ kubectl delete -f templates/kube2iam-rbac.yaml

You should also delete the inline policy `eks-sts` from your EKS worker node.
