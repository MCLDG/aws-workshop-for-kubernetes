...
This chapter explains how to apply Nginx and AWS ALB Ingress Controllers to a Kubernetes cluster.
...
It is also possible to use an AWS NLB in Kubernetes. We will devote a short section to this below.
...
Deploy an application:
  ...
  kubectl apply -f templates/echo-deployment.yaml

Deploy an NLB that routes traffic to the pods:

  kubectl apply -f templates/echo-nlb.yaml

Once the NLB is accepting traffic, enter the NLB DNS in a browser window.

Cleanup

  kubectl delete -f templates/echo-nlb.yaml
